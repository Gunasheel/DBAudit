...
StdLib.prototype.generateTokenNK = function (ip, userAgent, loginId, mobileIsd, APNS_Id, GCM_Id, secretKey, deviceType,deviceName, callBack) {
    var _this = this;
    /////////////////////////////////////////////////////////////////////

    // 1 -web, 2- app, 3- windows
    if (!deviceType) {
        deviceType = 1;
    }
    var deviceMapping = {
    ...
    var tokenGenQueryParams = _this.db.escape(ip) + ',' + _this.db.escape(userAgent)
        + ',' + _this.db.escape(loginId) + ',' + _this.db.escape(mobileIsd) + ',' + _this.db.escape(deviceType) + ',' + _this.db.escape(APNS_Id) + ',' + _this.db.escape(GCM_Id) + ',' + _this.db.escape(secretKey)+ ',' + _this.db.escape(deviceName);
    var tokenGenQuery = 'CALL pGenerateTokenNewNK(' + tokenGenQueryParams + ')';

    console.log(tokenGenQuery);

    _this.db.query(tokenGenQuery, function (err, results) {
        console.log("token gen error ",err)
        if (err) {
            callBack(err, null);
        }
        else {
...
StdLib.prototype.generateTokenSocialMedia = function (ip, userAgent, loginId, mobileIsd, APNS_Id, GCM_Id, secretKey, callBack) {
    var _this = this;
    /////////////////////////////////////////////////////////////////////
    ...
    var tokenGenQueryParams = _this.db.escape(ip) + ',' + _this.db.escape(userAgent)
        + ',' + _this.db.escape(loginId) + ',' + _this.db.escape(mobileIsd) + ',' + _this.db.escape(deviceType) + ',' + _this.db.escape(APNS_Id) + ',' + _this.db.escape(GCM_Id) + ',' + _this.db.escape(secretKey);
    var tokenGenQuery = 'CALL pGenerateTokenNewSocialMedia(' + tokenGenQueryParams + ')';
    ...
    try {
        ...
        if (token) {
            ...
            _this.db.query(validateTokenQuery, function (err, sessionResult) {
                console.log("token validation err", err);
                if (!err) {
...
StdLib.prototype.uploadDocumentToCloud = function (uniqueName, readStream, callback) {
    var remoteWriteStream = bucket.file('icanrefer/' + uniqueName).createWriteStream();
...
StdLib.prototype.deleteDocumentFromCloud = function (uniqueName) {
    var file = bucket.file('icanrefer/' + uniqueName);
